...
import { integrationCredentials, isEcPlatform, serviceClient } from "@/lib/ec-integrations"
...
async function refreshIfNeeded(platform: "base" | "shopify" | "square", config: any) {
  if (platform === "shopify" || !config.expires_at || new Date(config.expires_at).getTime() - Date.now() > 60 * 60 * 1000) return config.access_token
  if (!config.refresh_token) throw new Error("Reconnect required")
  const credentials = integrationCredentials(platform)
  const endpoint = platform === "base" ? "https://api.thebase.in/1/oauth/token" : (process.env.SQUARE_ENVIRONMENT === "sandbox" ? "https://connect.squareupsandbox.com/oauth2/token" : "https://connect.squareup.com/oauth2/token")
  const response = await fetch(endpoint, platform === "base" ? { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ grant_type: "refresh_token", client_id: credentials.clientId!, client_secret: credentials.clientSecret!, refresh_token: config.refresh_token }) } : { method: "POST", headers: { "Content-Type": "application/json", "Square-Version": "2026-01-22" }, body: JSON.stringify({ grant_type: "refresh_token", client_id: credentials.clientId, client_secret: credentials.clientSecret, refresh_token: config.refresh_token }) })
  ...
  await serviceClient().from("shop_api_configs").update({ access_token: token.access_token, refresh_token: token.refresh_token || config.refresh_token, expires_at: expiresAt, updated_at: new Date().toISOString() }).eq("id", config.id)
...
async function fetchProducts(platform: "base" | "shopify" | "square", token: string, config: any): Promise<ProductRow[]> {
  ...
  const response = await fetch(`${host}/v2/catalog/list?types=ITEM,IMAGE`, { headers: { Authorization: `Bearer ${token}`, "Square-Version": "2026-01-22", "Content-Type": "application/json" } })
...
export async function POST(_request: Request, { params }: { params: Promise<{ platform: string }> }) {
  ...
  try {
    ...
    await service.from("shop_products").update({ is_active: false }).eq("user_id", user.id).eq("platform_type", rawPlatform)
    if (products.length) {
